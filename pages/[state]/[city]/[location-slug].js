...
export async function getStaticProps({ params }) {
  try {
    ...
    const targetCitySlug = params.city;
    
    let foundLocation = null;
    
    for (const location of state.locations) {
      // Generate the same slug logic as in getStaticPaths
      const citySlug = (location.city || 'location').toLowerCase().replace(/\s+/g, '-');
      const locationSlug = (location.company + '-' + (location.address || 'center'))
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
      ...
      if (citySlug === targetCitySlug && locationSlug === targetLocationSlug) {
        foundLocation = {
          ...location,
          id: state.locations.indexOf(location) // Add an ID for consistent generation
        };
        break;
      }
    }
